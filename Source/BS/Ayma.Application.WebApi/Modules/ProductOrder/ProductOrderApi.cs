...
namespace Ayma.Application.WebApi.Modules.ProductOrder
{
    ...
    public class ProductOrderApi : BaseApi
    {
        ...
        private Response SyncOrder(dynamic arg)
        {
            ...
            if (string.IsNullOrWhiteSpace(order.P_OrderNo.Trim()))
            {
                return Fail("生产订单号为空");
            }
            ...
            {
                if (orderItem.P_Qty <= 0)
                {
                    return Fail("数量必须要大于0");
                }
                if (order.P_OrderNo != orderItem.P_OrderNo)
                {
                    return Fail("订单号不一致");
                }
                if (string.IsNullOrWhiteSpace(orderItem.P_GoodsCode))
                {
                    return Fail("物料编码为空");
                }
            }
            ...
            var mesOrderDetail =
                DataHelper.Map<List<ProductOrderDetail>, List<Mes_ProductOrderDetailEntity>>(order.OrderDetails);
