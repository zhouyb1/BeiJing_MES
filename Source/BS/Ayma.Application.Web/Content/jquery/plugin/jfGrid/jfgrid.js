...
(function ($, ayma) {
    ...
    $.extend($.jfGrid, {
        ...
        renderStatisticsOne: function ($self, name) {
            ...
            if (dfop.isStatistics) {
                ...
                $statistics.find('[colname="' + name + '"]').html(vaule.toFixed(2));
        ...
        reloadPage: function (e) {
            ...
            if ($et.hasClass('pagebtn')) {
                ...
                if (page != dfop.pageparam.page) {
                    $("#jfgrid_all_cb_girdtable").removeAttr("checked");
        ...
        renderData: function ($self) {
            ...
            if (dfop.inputCount != 0) {
                //绑定输入框上下左右键
                $self.jfGridInputFocus(dfop.inputCount);
            }
            dfop = null;
        },
                ...
                switch (node.editType) {
                    ...
                    case 'dateinput'://日期输入框
                        ...
                        if (!!node.fmt) {
                            ...
                            $dateinput = $('<input  type="text" class="form-control am-input-wdatepicker" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\'})" />').val(top.ayma.formatDate(value, 'yyyy-MM-dd') || now);
                        }
                    ...
                    case 'label':
                        if (!!node.numbercount) {
                            if (value != undefined) {
                                value = parseFloat(value).toFixed(node.numbercount);
                            }
                        }
        ...
        selectRow: function ($this, $et, dfop) {
            ...
            if (dfop.onSelectRow) {
                var datapath = $this.find('.jfgrid-hide-cell[rownum="' + rowid + '"]').attr('datapath');
                var row = $.jfGrid.getRowDataByPath(dfop, datapath);
                dfop.onSelectRow($.jfGrid.getSelectedRow($this), row, rowid);
            }
        },
    ...
    $.fn.jfGridInputFocus = function (rowInputCount) {
        ...
        $(".cGridInput").on("keydown", function (evt) {
            if (evt.which == 37 || evt.which == 38 || evt.which == 39 || evt.which == 40 || evt.which == 13) {
                ...
                if (tabIndex > 0) {
                    ...
                    $(".cGridInput[tabindex=" + tabIndex + "]")[0].select();
                    return false;
                }
                return true;
            }
