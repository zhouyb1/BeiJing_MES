...
var bootstrap = function ($, ayma) {
    ...
    var page = {
        ...
        initGird: function () {
            $('#girdtable').jfGrid({
                url: top.$.rootUrl + '/MesDev/Mes_SaleManager/GetGoodsList',
                headData: [
                ...
                onSelectRow: function (rowdata, row, rowid) {
                    ...
                    if (isChecked.is(":checked")) {
                        ...
                        var quantity = ($("#quantity").val()) == "" ? "0" : $("#quantity").val();
                        //copy需要更改的地方
                        row['S_GoodsCode'] = row['I_GoodsCode'];
                        row['S_GoodsName'] = row['I_GoodsName'];
                        row['S_SupplyCode'] = row['I_SupplyCode'];
                    ...
                    if (JSON.stringify(rowslist) !== '[]') {
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            for (var j = 0; j < rowlistlenght; j++) {
                                if (rows[i]['I_GoodsCode'] == rowslist[j]['S_GoodsCode'] && rows[i]["I_Batch"] == rowslist[j]["S_Batch"]) {
