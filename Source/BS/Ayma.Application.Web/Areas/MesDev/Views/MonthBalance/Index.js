        ...
        bind: function() {
                ...
                function() {
                    ...
                    if (ayma.checkrow(month)) {
                        if (status == 1) {
                            ...
                            ayma.layerConfirm('是否确认月结？',
                                function(res) {
                                    if (res) {
                                        ayma.postForm(top.$.rootUrl + '/MesDev/MonthBalance/PostOrCancel',
                                            { month: month, type: 1 },
                                            function() {
                                                refreshGirdData();
                                            });
                ...
                function() {
                    ...
                    if (ayma.checkrow(month)) {
                        if (status == 2) {
                            ...
                            ayma.layerConfirm('是否确认反月结？',
                                function(res) {
                                    if (res) {
                                        ayma.deleteForm(top.$.rootUrl + '/MesDev/MonthBalance/PostOrCancel',
                                            { month: month, type: 2 },
                                            function() {
                                                refreshGirdData();
                                            });
            ...
            $('#am_delete').on('click',
                function() {
                    var keyValue = $('#girdtable').jfGridValue('ID');
                    if (ayma.checkrow(keyValue)) {
                        ayma.layerConfirm('是否确认删除该项！',
                            function(res) {
                                if (res) {
                                    ayma.deleteForm(top.$.rootUrl + '/MesDev/MonthBalance/DeleteForm',
                                        { keyValue: keyValue },
                                        function() {
                                            refreshGirdData();
                                        });
