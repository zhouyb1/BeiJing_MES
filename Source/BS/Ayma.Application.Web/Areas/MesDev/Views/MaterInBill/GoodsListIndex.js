                    ...
                    if (JSON.stringify(rowslist) !== '[]') {
                        ...
                        for (var i = 0; i < rows.length; i++) {
                            for (var j = 0; j < rowlistlenght; j++) {
                                if (rows[i]['p_goodscode'] == rowslist[j]['M_GoodsCode'] && rows[i]['p_supplycode'] == rowslist[j]['M_SupplyCode'] && ayma.formatDate(batch, "yyyy-MM-dd").toString().replace(/-/g, "") == rowslist[j]['M_Batch']) {
